...
def gpu_searchsorted_scan(P, X):
    ...
    (_, B), _ = theano.scan(gpu_searchsorted_step, outputs_info=[T.zeros_like(X, dtype='int64'), T.ones_like(X, dtype='int64')*(P.shape[0]-1)], non_sequences=[X, P], n_steps=N, allow_gc=True)
    return B[-1]
